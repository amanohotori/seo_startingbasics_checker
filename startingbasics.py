...
def check_seo_criteria(url):
    ...
    if title_tag is None:
        ...
        if len(title_text_z2h) >= 30 and len(title_text_z2h) <= 60:
            print("文字数が30~60文字の場合は満点")
            score += 10
        elif len(title_text_z2h) > 60:
            print("文字数が60文字より多い場合は5点")
            score += 5
        else:
            print("文字数が30文字未満の場合は0点")
            score += 0
    ...
    if meta_tag is None:
        ...
        if len(meta_text_z2h) >= 70 and len(meta_text_z2h) <= 155:
            print("文字数が70~155文字の場合は10点")
            score += 10
        elif len(meta_text_z2h) > 155:
            print("文字数が155文字より多い場合は5点")
            score += 5
        else:
            print("文字数が70文字未満の場合は0点")
            score += 0
    print(f"現在のスコア : {score}/100")
    ...
    if h1_tags is None:
       print("ヘッダータグ<h1>が存在しない場合は0点")
       score += 0
    elif not h1_tags:
        print("設定されたヘッダータグが空の場合は0点")
        score += 0
    else:
        print("メタ説明タグが存在する場合は文字数をチェック")
        ...
        if len(h1_text_z2h) >= 5 and len(h1_text_z2h) <= 60:
            print( "文字数が5~60文字の場合は満点")
            score += 10
        elif len(h1_text_z2h) > 60:
            print("文字数が60文字より多い場合は5点")
            score += 5
        else:
            print("文字数が5文字未満の場合は0点")
            score += 0
    print(f"現在のスコア : {score}/100")

    print("5. バリアフリーのため画像に alt （短い要約）が適切に付与されているかのチェック）(10点)")
    img_tags = soup.find_all('img')
    if not img_tags:
        print("画像が存在しなければ 10点（減点なし）")
        score += 10
    else:
        alt_counts = 0
        ...
        if alt_counts == len(img_tags):
            print("画像の数がalt_countsと同じ（全部に2文字以上のaltがあれば）10点")
            score += 10
        else:
            print("altのない画像があれば0点")
            score += 0
    print(f"現在のスコア : {score}/100")
    ...
    if meta_viewport:
        if 'width=device-width' in meta_viewport.get('content', ''):
            print("meta_viewportタグがあって、device-widthが指定されていれば10点")
            score += 10
        else:
            print("meta_viewportタグがない、または、device-widthが指定されていなければ0点")
            score += 0
    print(f"現在のスコア : {score}/100")
    ...
    for link in social_links:
        ...
        if href and (href.startswith('https://twitter.com/') or href.startswith('https://www.facebook.com/')):
            print("Twitter または Facebook へのリンクが一つ以上存在する (5点)")
            score += 5
            break
        else:
            score += 0
